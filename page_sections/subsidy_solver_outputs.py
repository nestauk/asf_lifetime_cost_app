...
def render_required_subsidy_chart_section(required_subsidy_df: pd.DataFrame) -> None:
    ...
    with st.container(border=True):
        st.markdown(
            '<div style="font-weight:700; color:#0F294A; font-size:16px; margin-bottom:2px;">'
            "Subsidy pathway needed for cost parity</div>",
            unsafe_allow_html=True,
        )
        st.markdown(
            '<div style="font-size:13px; color:#666; margin-bottom:16px;">'
            "What level of subsidy is needed in each year of installation for the heat pump to have the same annualised lifetime cost as the gas boiler. <br> The dashed line is the current BUS subsidy level for reference. </div>",
            unsafe_allow_html=True,
        )
    ...
    with st.container(border=True):
        ...
        for _, row in required_subsidy_df.iterrows():
            ...
            if required_subsidy < 0:
                ...
                message = "More than the installation cost - subsidy alone can't close the gap"
